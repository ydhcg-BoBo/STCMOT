...
class MCJDETracker(object):
    ...
    def update_tracking(self, pre_im_blob, im_blob, img_0):
        """
        :param im_blob:
        :param img_0:
        :return:
        ...
        with torch.no_grad():
            output = self.model.forward(pre_im_blob, im_blob)[-1]
        ...
        # translate and scale
        # dets 包含了6个信息，前四个位置信息和第五个置信度，第六个
        ...
        for cls_id in range(self.opt.num_classes):  # cls_id从0开始
            ...
            second_tracked_tracks = [r_tracked_tracks[i] for i in u_track
                                                  if r_tracked_tracks[i].state == TrackState.Tracked]
            dists = matching.iou_distance(second_tracked_tracks, cls_detects_second)
